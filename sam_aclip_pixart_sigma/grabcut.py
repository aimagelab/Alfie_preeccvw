...
def grabcut(image, attention_maps, image_size, sure_fg_threshold, maybe_fg_threshold, maybe_bg_threshold,
    ...
    for attention_map in attention_maps:
        ...
        maybe_fg_full_mask += ((attention_map > threshold_maybe_fg) & (attention_map <= threshold_sure_fg))
        maybe_bg_full_mask += ((attention_map > threshold_maybe_bg) & (attention_map <= threshold_maybe_fg))
        sure_bg_full_mask += (attention_map <= threshold_maybe_bg)

    mask = torch.zeros((image_size, image_size), dtype=torch.uint8)
    mask = torch.where(sure_bg_full_mask, cv2.GC_BGD, mask)
    mask = torch.where(maybe_bg_full_mask, cv2.GC_PR_BGD, mask)
    mask = torch.where(maybe_fg_full_mask, cv2.GC_PR_FGD, mask)
    mask = torch.where(sure_fg_full_mask, cv2.GC_FGD, mask)

    bgdModel = np.zeros((1, 65), np.float64)
    fgdModel = np.zeros((1, 65), np.float64)
    mask = mask.numpy().astype(np.uint8)
    # mask_vis = mask * 85
    # vis = torch.where(torch.from_numpy(mask[:, :, None]), torch.from_numpy(np.array(image)), 0).numpy()
    try:
    ...
    # if opening_mask_factor > 0:
    #     kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (opening_mask_factor, opening_mask_factor))
    #     alpha = cv2.morphologyEx(alpha, cv2.MORPH_OPEN, kernel, iterations=5)
